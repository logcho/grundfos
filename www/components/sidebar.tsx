...
export function Sidebar({ buildings, activeBuildingId, onScanBuilding }: SidebarProps) {
  ...
  return (
    <aside className="flex h-full w-full flex-col border-r border-slate-200 bg-white/95 p-0 backdrop-blur lg:w-[420px] shrink-0 z-10 shadow-xl">
        ...
        {buildings.map((b) => {
          ...
          return (
            <div 
              ...
              className={`relative z-10 group cursor-pointer overflow-hidden rounded-2xl border transition-all duration-300 ml-8 ${
                isActive ? 'border-cyan-400 shadow-[0_8px_30px_rgb(0,0,0,0.08)] ring-1 ring-cyan-400 bg-white scale-[1.02]' 
            ...
            >
              ...
              <div className={`absolute -left-10 top-6 h-3 w-3 rounded-full border-2 border-white ring-4 transition-colors ${
                isActive ? 'bg-cyan-500 ring-cyan-100' : 'bg-slate-300 ring-slate-100 group-hover:bg-cyan-300'
              }`} />
              ...
              <div className="flex items-center justify-between border-b border-slate-50/80 p-4 pb-3">
                ...
                <div className={`flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[10px] font-bold tracking-wider uppercase ${
                  isScanning ? 'bg-amber-100 text-amber-700 animate-pulse ring-1 ring-amber-200' :
              ...
              {/* Card Body */}
              <div className="p-4 pt-3 flex items-center justify-between bg-white relative">
                <div>
                  <p className="text-[9px] uppercase tracking-[0.1em] text-slate-400 font-bold mb-1">Usable Roof Space</p>
                  <p className="text-sm font-bold text-slate-700">
                    {b.spatial_data ? b.spatial_data.area_sqft.toLocaleString() : Math.round(b.area_sqm * 10.7639).toLocaleString()} <span className="text-[10px] font-medium text-slate-400">sqft</span>
                  </p>
                </div>
              ...
              {b.cv_data && (
                 <div className={`px-4 py-2 text-[10px] font-bold tracking-wide uppercase flex justify-between items-center ${
                   b.cv_data.cooling_tower_present ? 'bg-emerald-50 text-emerald-700 border-t border-emerald-100' : 'bg-slate-50 text-slate-500 border-t border-slate-100'
                 }`}>
