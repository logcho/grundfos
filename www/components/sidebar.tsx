    ...
    <aside className="flex h-[50dvh] lg:h-full w-full flex-col border-t lg:border-t-0 lg:border-r border-slate-200 bg-white/95 p-0 backdrop-blur lg:w-[420px] shrink-0 z-20 shadow-[0_-10px_40px_rgba(0,0,0,0.1)] lg:shadow-xl order-2 lg:order-1">
      ...
      <div className="flex-1 overflow-y-auto bg-slate-50/50 p-4 space-y-3 relative">
        ...
        {buildings.map((b) => {
          ...
          const isViable = b.status === "Viable" || b.status === "Prime Target";
          const isRejected = b.status === "Rejected";
          const isPrime = b.status === "Prime Target";
          ...
          return (
            ...
            >
              ...
              <div className={`absolute -left-10 top-6 h-3 w-3 rounded-full border-2 border-white ring-4 transition-colors ${
                ...
                isPrime ? 'bg-amber-600 ring-amber-100' :
              ...
              <div className="flex items-center justify-between border-b border-slate-50/80 p-4 pb-3">
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-0.5">
                    <p className="text-[10px] text-slate-600 font-mono opacity-70">UUID: {b.id.substring(0, 8).toUpperCase()}</p>
                    {b.viability_score !== undefined && (
                      <span className="text-[10px] bg-slate-100 px-1.5 py-0.5 rounded font-bold text-slate-700">
                        SCORE: {b.viability_score}
                      </span>
                    )}
                  </div>
                </div>
                <div className={`flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[10px] font-bold tracking-wider uppercase ${isScanning ? 'bg-amber-100 text-amber-700 animate-pulse ring-1 ring-amber-200' :
                  isPrime ? 'bg-amber-600 text-white ring-1 ring-amber-700' :
                  ...
                  {isPrime && <CheckCircle2 className="h-3 w-3 text-white" />}
                ...
                <div className="text-right ml-auto">
                  ...
                  {b.spatial_data && (
                    <p className="text-xs font-bold text-emerald-600 mt-0.5">
                      +${calculateFinancialROI(b.spatial_data.annual_yield_gallons).annualSavings.toLocaleString()} <span className="text-[9px] font-medium text-emerald-600/70 uppercase tracking-wider">/yr ROI</span>
                    </p>
                  )}
                </div>
              </div>
